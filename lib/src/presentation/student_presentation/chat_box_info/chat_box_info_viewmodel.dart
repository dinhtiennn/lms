...
class ChatBoxInfoViewModel extends BaseViewModel {
  ...
  void toChatBoxMember(ChatBoxModel chatboxParam) async {
    try {
      dynamic result = await Get.toNamed(Routers.chatBoxMember, arguments: {'chatBox': chatboxParam});
      if (result != null) {
  ...
  void reNameChatBox(String newName) async {
    try {
      if (chatbox.value?.id == null) {
        showToast(title: 'Không thể đổi tên nhóm chat không xác định', type: ToastificationType.error);
        return;
      }

      if (newName.isEmpty) {
        showToast(title: 'Tên nhóm không được để trống', type: ToastificationType.error);
        return;
      }

      // Kiểm tra xem người dùng hiện tại có quyền đổi tên nhóm chat không
      if (chatbox.value?.createdBy != currentUserEmail) {
        showToast(title: 'Bạn không có quyền đổi tên nhóm chat này', type: ToastificationType.error);
        return;
      }

      // Gọi API đổi tên nhóm chat
      NetworkState resultReName =
          await chatBoxRepository.reNameChatBox(chatBoxId: chatbox.value?.id ?? '', newName: newName);
      ...
      if (resultReName.isSuccess) {
        ...
        showToast(title: 'Đã đổi tên nhóm chat thành công', type: ToastificationType.success);
      } else {
        showToast(title: 'Không thể đổi tên nhóm chat. Vui lòng thử lại sau.', type: ToastificationType.error);
      }
    } catch (e) {
