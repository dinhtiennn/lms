...
import 'package:lms/src/presentation/widgets/date_picker/src/date_format.dart';
...
abstract class BasePickerModel {
  ...
  //return left divider (phân cách) string
  String leftDivider();

  //return right divider string
  String rightDivider();

  //layout proportions (tỷ lệ) for 3 columns
...
class DatePickerModel extends CommonPickerModel {
  ...
  void _fillMiddleLists() {
    // Đảm bảo `minMonth` và `maxMonth` được tính toán chính xác dựa trên `currentTime.year`
    int minMonth = _minMonthOfCurrentYear();
  ...
  void _fillRightLists() {
    // Tạo danh sách năm từ `minTime.year` đến `maxTime.year`
  ...
  int _minDayOfCurrentMonth() {
    return (currentTime.year == minTime.year && currentTime.month == minTime.month)
        ? minTime.day
        : 1;
  }
  ...
  void setRightIndex(int index) {
    ...
    // Cập nhật `currentTime.year` và đảm bảo ngày/tháng hợp lệ
    int destYear = minTime.year + index;
    int dayCount = calcDateCount(destYear, currentTime.month);

    DateTime newTime = currentTime.isUtc
        ? DateTime.utc(destYear, currentTime.month, currentTime.day <= dayCount ? currentTime.day : dayCount)
        : DateTime(destYear, currentTime.month, currentTime.day <= dayCount ? currentTime.day : dayCount);

    // Kiểm tra giới hạn minTime và maxTime
    ...
    // Cập nhật lại danh sách tháng và ngày theo năm mới
    _fillMiddleLists();
    _fillLeftLists();

    _currentMiddleIndex = currentTime.month - _minMonthOfCurrentYear();
    _currentLeftIndex = currentTime.day - _minDayOfCurrentMonth();
  }

  @override
  void setMiddleIndex(int index) {
    super.setMiddleIndex(index);
    // Cập nhật `currentTime.month` và đảm bảo ngày hợp lệ
    int minMonth = _minMonthOfCurrentYear();
    int destMonth = minMonth + index;
    int dayCount = calcDateCount(currentTime.year, destMonth);

    DateTime newTime = currentTime.isUtc
        ? DateTime.utc(currentTime.year, destMonth, currentTime.day <= dayCount ? currentTime.day : dayCount)
        : DateTime(currentTime.year, destMonth, currentTime.day <= dayCount ? currentTime.day : dayCount);
    ...
    // Cập nhật lại danh sách ngày theo tháng mới
    _fillLeftLists();
    _currentLeftIndex = currentTime.day - _minDayOfCurrentMonth();
  }
